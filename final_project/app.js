...
places.features.forEach(function(marker) {
    ...
    var popup = new mapboxgl.Popup({
                        offset: 50,
                        closeButton: false})
                .setHTML('<strong>' + marker.properties.title + '</strong> <p>' + marker.properties.description + 
                         '</p> <strong>Aksesibilitas: </strong> <p>' + marker.properties.Accessibility + '</p>');
    ...
    if(!(tourAccess.includes(marker.properties.filter))){
        tourAccess.push(marker.properties.filter);
